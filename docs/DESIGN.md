# Design Document: How Brittle Are Activation Monitors Under Distribution Shift

## 1. Purpose

This document is the method contract for **How Brittle Are Activation Monitors Under Distribution Shift**. It specifies what is
...
One-liner: Stress-test activation monitors for deception/harm signals under shift and simple adaptive attacks.

Hypothesis: Stress-test activation monitors for deception/harm signals under shift and simple adaptive attacks.
...
|---|---|---|
| Config schema | `actmon.configs.schema` | Typed dataclasses + role-keyed models |
| Loader | `actmon.configs.loader` | Hydra compose API |
| Device | `actmon.models.device` | MPS/CUDA/CPU resolution + fallback flag |
| Registry | `actmon.models.registry` | Pinned revisions + hardware fit |
| Hooks | `actmon.models.hooks` | Capture, steer, ablate, patch, attn intervene |
| Generation | `actmon.models.generation` | Chat template path recording |
| Data | `actmon.data` | Synthetic items, splits, manifests |
| Evaluation | `actmon.evaluation` | Metrics, failure harness, protocols |
| Ablation | `actmon.ablation` | Control / dropout / seed-sweep arms |
| Reporting | `actmon.reporting` | Aggregate → MD/TeX/figures |
| Cache | `actmon.cache` | Versioned artifact store |
...
`src/actmon/actmon/` are preserved across regeneration.
